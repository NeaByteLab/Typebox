...
When a `guard` is provided, `Define` returns a wrapper. On each call the wrapper runs every guard first and only then runs the contract. Each guard receives a deep-frozen [structured clone](https://developer.mozilla.org/en-US/docs/Web/API/Window/structuredClone) of the input when the input is an object, so a guard can read but never mutate the caller's value. The contract itself still runs on the original input. A failing guard throws a [`TypeError`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/TypeError) whose `message` joins the reasons and whose [`cause`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Error/cause) holds the full reason list. Callers detect it with `error instanceof Error && Array.isArray(error.cause)` and read the reasons from `error.cause`. See [Validation](Validation.md) for the verdict and rejection rules.
